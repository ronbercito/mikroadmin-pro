#!/usr/bin/env bash
# MikroAdmin Pro - Instalador/Actualizador para Debian 12
set -e
...
# === MODO ACTUALIZACIÓN ===
# Si ya existe una instalación previa, actualizar en lugar de instalar desde cero.
if [ -d "$APP_DIR/backend" ] && [ -d "$APP_DIR/frontend" ] && [ -f "$APP_DIR/backend/.env" ]; then
  echo "=== MikroAdmin Pro - Actualización ==="
  echo "Instalación previa detectada en $APP_DIR"
